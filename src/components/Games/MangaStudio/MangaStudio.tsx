...
const MangaViewer: React.FC<MangaViewerProps> = ({ story, onContinue, onHome }) => {
...
const MangaStudio: React.FC = () => {
    ...
    const renderHome = () => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' }}>
            ...
            <button
                onClick={() => { loadStories(); setView('gallery'); }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%' }}>
            ...
            {stories.length === 0 ? (
                <div style={{ color: '#888', textAlign: 'center', padding: '20px' }}>
                    No manga created yet. Start creating!
                </div>
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {stories.map(story => (
                        <div
                            key={story.id}
                            onClick={() => { setCurrentStory(story); setView('viewer'); }}
                            style={{
                                background: '#3D1A1A',
                                border: '2px solid #5D4037',
                                borderRadius: '8px',
                        ...
                        >
                            ...
                            <div style={{ color: '#888', fontSize: '11px' }}>
                                {MANGA_STYLES.find(s => s.id === story.style)?.emoji} {story.pages.length} pages
                            </div>
                        </div>
                    ))}
                </div>
